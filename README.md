...
70. add search functionality at client(B)  
- add search route at App.jsx 
```javascript
  ...
  ...
  ...
```
- create Search.jsx at pages folder in client(B)  
```javascript
  ...
  export default function Search() {
    const [sidebarData, setSidebarData] = useState({
      searchTerm: '', sort: 'desc', category: 'uncategorized',});
    const [posts, setPosts] = useState([]);
    const [loading, setLoading] = useState(false);
    ...
    useEffect(() => {
      ...
      if (sortFromUrl) {setSidebarData({...sidebarData,sort: sortFromUrl,});}
      if (categoryFromUrl) {setSidebarData({...sidebarData,category: categoryFromUrl,});}
      const fetchPosts = async () => {
        try {
          setLoading(true);
    ...
    const handleShowMore = async () => {
      ...
      const newSearchQuery = urlParams.toString();
      const res = await fetch(`/api/post/getposts?${newSearchQuery}`);
      const data = await res.json();
      if (res.ok) {
...
```
- add search functionalty at Header.jsx at components folder in client(B)  
```javascript
  ...
  useEffect(() => {
    const urlParams = new URLSearchParams(location.search);
    const searchTermFromUrl = urlParams.get('searchTerm');
    if (searchTermFromUrl) {setSearchTerm(searchTermFromUrl);}}, [location.search]);
    ...
    const handleSubmit = (e) => {
    e.preventDefault();
    const urlParams = new URLSearchParams(location.search);
